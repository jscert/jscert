...
let get_value_ref state r =
	match JsInterpreterBisect.ref_get_value
		(JsInterpreterBisect.runs max_int)
		state (JsPreliminary.execution_ctx_initial false)
		(JsSyntax.Coq_resvalue_ref r) with
    | JsInterpreterBisect.Coq_result_some (
...
let _ =
  ...
  try
    ...
    in match JsInterpreterBisect.run_javascript
            (JsInterpreterBisect.runs max_int)
            exp'
    with
    | JsInterpreterBisect.Coq_result_some o ->
       begin
         match o with
    ...
    | JsInterpreterBisect.Coq_result_not_yet_implemented ->
        ...
        exit 2
    | JsInterpreterBisect.Coq_result_bottom s ->
