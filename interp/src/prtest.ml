...
let probject_properties_aux str p =
...
let probject_properties = probject_properties_aux ""

let probject_internal_aux s o =
  match o with
  {object_proto_ = vproto;
   object_class_ = sclass;
   object_extensible_ = _;
   object_properties_ = _;
   object_prim_value_ = valueop;
   object_get_ = _;
   object_get_own_prop_ = _;
   object_delete_ = _;
   object_define_own_prop_ = _;
   object_construct_ = _;
   object_call_ = _;
   object_has_instance_ = _;
   object_scope_ = _;
   object_formal_parameters_ = _;
   object_code_ = code_op;
   object_target_function_ = _;
   object_bound_this_ = _;
   object_bound_args_ = _;
   object_parameter_map_ = map_op;
  } ->
    s ^ "__proto__ " ^ (prvalue vproto) ^ "\n" ^
    s ^ "__class__ " ^ (string_of_char_list sclass) ^ "\n" ^
    (propt (s ^ "__param_map__ ") "\n" prloc map_op) ^
    (propt (s ^ "__code__ ") "\n" 
       (function Coq_funcbody_intro (_, s) -> string_of_char_list s) 
       code_op)

let prfieldmap loc obj =
  prloc loc ^ 
    (probject_internal_aux ("\t") obj) ^
    (probject_properties_aux ("\t") (obj.object_properties_))

let dontprint = function
  | (Coq_object_loc_prealloc s, _) -> s = Coq_prealloc_global
  | _ -> true
...
let prheap =
  ...
  (String.concat ""
  	  (List.rev (List.map (fun (key, v) ->
         Printf.sprintf "%s\n" (prfieldmap key v)) (List.filter dontprint (heap_to_list heap))
  	))) ^
  "}"
