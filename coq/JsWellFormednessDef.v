...
Lemma heap_extends_trans : forall {X Y:Type} (H H' H'':Heap.heap X Y),
                             heap_extends H' H ->
                             heap_extends H'' H' ->
                             heap_extends H'' H.
Proof.
...
Lemma state_extends_trans : forall (S S' S'':state),
                              state_extends S' S ->
                              state_extends S'' S' ->
                              state_extends S'' S.
Proof.
...
Inductive wf_res (S:state) (str:strictness_flag) : res -> Prop :=
  | wf_res_intro : forall (rv:resvalue) (lab:label), (*not sure about the label and the type*)
                     ...
                     wf_res S str {|res_type := restype_normal ;
                                    res_value := rv ;
                                    res_label := lab|}.
...
Inductive wf_attributes (S:state) (str:strictness_flag) : attributes -> Prop :=
  | wf_attributes_data_of : forall (v:value) (b b' b'':bool),
                              wf_value S str v ->
                              wf_attributes S str (attributes_data_of
                                                     {|attributes_data_value := v;
                                                       attributes_data_writable := b;
                                                       attributes_data_enumerable := b';
                                                       attributes_data_configurable := b''|})
  | wf_attributes_accessor_of : forall (v v':value) (b b':bool),
                                  wf_value S str v ->
                                  wf_value S str v' ->
                                  wf_attributes S str (attributes_accessor_of
                                                         {|attributes_accessor_get := v;
                                                           attributes_accessor_set := v';
                                                           attributes_accessor_enumerable := b;
                                                           attributes_accessor_configurable := b'|}).
