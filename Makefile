...
interp/src/interpreter.ml: coq/JsInterpreterExtraction.vo
	# The two following lines are here temporary.  They just replace all [raise Not_found] generated by Coq by a [failwith] followed by a line number, which is much easier to use for debuging purpose. -- Martin.
	cat interp/src/interpreter.ml | \
		awk '{gsub(/raise Not_found/,"failwith \""NR"\"",$$0);print}' \
		> interp/src/interpreter.ml.tmp
	mv interp/src/interpreter.ml.tmp interp/src/interpreter.ml
